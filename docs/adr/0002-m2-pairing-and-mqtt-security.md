...
> 2026-07-26 架构更新：ADR-0003 替代本文件中 NODE_ID“可迁移”和主板
> 更换复用原 NODE_ID 的条款。一次当前归属内的网络重配、凭据轮换和主机
> 恢复仍可保留 NODE_ID；显式退役、硬件更换或 retired hardware 重新配对
> 后必须使用全新 NODE_ID。其余 PoP、安全配对、双槽、Dynamic Security、
> TLS 和恢复边界继续有效。
...
现有真实节点从匿名实验身份迁入正式凭据时，可以在同一 HARDWARE_ID、
同一未退役归属内保留 `node_id=gh-n1-a9f2f8`，避免 Home Assistant
创建第二台设备。该一次性认证迁移不授权 NODE_ID 跨硬件复用。
