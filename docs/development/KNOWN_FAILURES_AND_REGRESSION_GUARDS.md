...
| KF-046 | FC4 N3-W setup AP discovery / secret-safe serial diagnosis | F3:50 scoped NVS reset 后操作者按板卡尾号寻找 Wi-Fi，未发现对应热点；诊断过滤器只屏蔽 `GHN3W2` payload，却让另一条含 raw pairing ID 的普通状态日志进入受控工具输出 | 绑定固件使用通用 SSID `Greenhouse N3-W Setup`，不是 MAC 尾号；fallback AP timeout 为 90 秒，操作提示没有冻结真实 SSID/窗口。串口日志存在不带 `GHN3W2` 前缀但含 `pairing_id=` 的身份行，单一 payload-prefix 过滤不完整 | 当前 live workaround 必须先绑定生成配置中的 exact SSID/timeout，复位后在明确窗口内连接；任何串口诊断整行拒绝 `GHN3W2`、`pairing_id=`、setup-secret/password/PSK 类字段，仅发布哈希和长度。后续 source 修复应为多板提供不泄露身份的可区分 setup SSID/提示并加入 negative redaction regression；受控输出中的 raw pairing ID 不进入文件或 GitHub | OPEN |
