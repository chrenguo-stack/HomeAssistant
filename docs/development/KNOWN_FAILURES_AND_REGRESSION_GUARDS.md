...
- **Wi-Fi failure truth boundary**：`MQTT/application uplink failure != real STA Wi-Fi loss`。真实 Wi-Fi loss 必须绑定实际 STA disconnected/unassociated 与 Direct IP path unavailable；真实 Wi-Fi recovery 必须绑定 STA/IP 恢复与自动回到 Direct。
