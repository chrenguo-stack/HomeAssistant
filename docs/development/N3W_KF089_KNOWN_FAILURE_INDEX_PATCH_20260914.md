...
Status: `PUBLIC_SAFE_EXACT_EDIT_INTENT`
Date: `2026-09-14`

This file freezes the exact semantic update that must be applied to `docs/development/KNOWN_FAILURES_AND_REGRESSION_GUARDS.md` when the stacked KF-089 closeout is integrated. It exists so the accepted 2026-09-14 live result is not trapped in chat even if the large central index is edited later with a normal local exact-base workflow.

## Domain table replacement

Replace the current KF-089 description with:
...
Replace the existing KF-089 quick-index row with:
...
Add the following rules to the fixed regression section:
...
Apply this edit only from a fresh exact-base read of the then-current central index. Do not reconstruct the large file from a truncated view. After editing, run public-repository safety and a focused diff review proving only the intended KF-089/domain/regression-rule hunks changed.

```text
KNOWN_FAILURE_CURRENT_STATUS=GUARDED
KF089_END_TO_END_RELAY_TELEMETRY=PROVEN
CENTRAL_INDEX_EDIT_PENDING=true
```
