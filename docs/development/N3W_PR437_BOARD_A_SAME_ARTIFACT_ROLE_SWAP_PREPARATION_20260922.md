...
Status: `BOARD_A_SAME_ARTIFACT_WRITE_PASS; BOARD_A_BATTERY_DIRECT_REBASELINE_PASS; BOARD_A_TO_BOARD_B_ROLE_SWAP_PASS; BOARD_A_RELAY_CONTINUITY_600S_PASS; BOARD_A_RELAY_TO_DIRECT_FAILBACK_PASS; ROLE_SWAP_ACCEPTANCE_PASS`
...
## 2026-09-22 battery-boot role-swap execution entry

Fresh entry state immediately before the physical movement gate:

```text
PR471_HEAD_BEFORE_ENTRY=30f080dd1ffebbec631a87d0f28cb4d7cff5d618
...
TRANSITION_TIMEOUT_SECONDS=240
RELAY_GATEWAY_MUST_EQUAL_BOUND_BOARD_B=true
POST_RELAY_REQUIRED_SEQ_ADVANCE=2
...
The observer must stop after the Direct -> Relay role-swap classification. On PASS, Board A stays in the Relay-child location and Board B stays in the Direct/Gateway location for the separately authorized 600-second Relay continuity gate.


## 2026-09-22 Board A same-boot Direct -> Relay role-swap closure

The battery-powered Board A was moved from the validated Direct/Wi-Fi position to the previously validated Relay-child position while Board B remained stationary as the Direct/Gateway side.
...
N3W_PR437_BOARD_A_SAME_BOOT_DIRECT_TO_RELAY_ROLE_SWAP=PASS
ROLE_SWAP_GATE_RESULT=PASS
```

The two missing transition-boundary sequences, 179-180, are retained as measured evidence. Under the frozen Option-B contract this does not fail the role-swap gate because the product does not claim end-to-end every-sample delivery across a real transport-attempt failure boundary. This mirrors the already-recorded behavior of the opposite role direction, where one and three transition-boundary sequences were missing in two independent runs.

```text
A_AS_CHILD_B_AS_GATEWAY_DIRECT_TO_RELAY=PASS
ROLE_SYMMETRY_FUNCTIONAL_EVIDENCE=PASS
TRANSITION_BOUNDARY_ZERO_LOSS=NOT_GUARANTEED
END_TO_END_EVERY_SAMPLE_DELIVERY=false
```

The physical topology must now remain unchanged for the next gate:

```text
BOARD_A_POSITION=RELAY_CHILD_LOCATION
BOARD_B_POSITION=DIRECT_GATEWAY_LOCATION
BOARD_A_REBOOT=false
BOARD_B_REBOOT=false
...
## 2026-09-22 Board A same-boot Relay -> Direct failback closure

Fresh failback evidence:
...
N3W_PR437_BOARD_A_SAME_BOOT_RELAY_TO_DIRECT_FAILBACK=PASS
FAILBACK_GATE_RESULT=PASS
```

This closes the reversed-role same-boot round trip.

```text
A_AS_CHILD_B_AS_GATEWAY_DIRECT_TO_RELAY=PASS
A_AS_CHILD_B_AS_GATEWAY_RELAY_CONTINUITY_600S=PASS
A_AS_CHILD_B_AS_GATEWAY_RELAY_TO_DIRECT=PASS
A_AS_CHILD_B_AS_GATEWAY_SAME_BOOT_ROUND_TRIP=PASS
ROLE_SWAP_ACCEPTANCE=PASS
```

The role-swap acceptance retains the frozen Option-B boundary: Direct -> Relay transition loss can occur. The observed reversed-role Direct -> Relay boundary lost seq 179-180; the reversed-role Relay steady-state and Relay -> Direct failback had zero missing sequences.
