...
- worker 正在运行时拒绝新的配对请求；
- 多 Manager 仍返回 `selection_required`，选择动作只能在 worker 空闲时执行；
- 状态快照不包含 pairing secret、方向密钥、MQTT 密码或凭据正文。

当前取消为协作式：在发现阶段和安全闭环边界检查取消标志；正在执行的单个底层网络调用依靠 Stage 2C-2 的超时上限返回。成功完成的闭环不因最后时刻到达的取消请求被错误回滚。
...
现有生产 RC2 YAML 和 Stage 2C-2 组件接口保持不变。
...
7. 恢复时 active marker 必须与一个有效 `COMMITTED` 槽精确匹配；
8. 另一槽存在相同或更高 committed generation 时失败关闭；
9. rollback 清除候选元数据，不改变现有 active generation。
...
任一条件失败进入 `FAILED`，随后只能 rollback；旧 active generation 在激活前始终保持不变。
...
- host C++ async、journal、MQTT activation 合同测试；
