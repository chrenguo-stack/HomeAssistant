...
- keyed HMAC-SHA256 payload digest、固定 schema、slot、state、generation 和长度绑定；
- 凭据确定性二进制编码与严格解码；
...
6. 记录根密钥通过 `gh-persist-encryption-v1` 和 `gh-persist-digest-v1` 两个 domain 分别派生 encryption key 与 digest key；
7. digest 使用 HMAC-SHA256，不在信封中暴露可用于离线猜测的裸明文 SHA-256；
...
- 最小板非生产目标；
- 完整 RC2 产品板非生产目标；
- NVS、HMAC、HMAC-SHA256、ChaCha20-Poly1305 源文件编译和链接；
...
- 完整旧 NVS 快照重放的防回滚锚点；当前 marker 与 generation 只能检测单次存储内的冲突，不能阻止攻击者恢复一份完整、内部自洽的旧快照；
- OTA 跨 schema 迁移；
