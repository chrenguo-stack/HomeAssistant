...
CONF_AUTH_FAILURE_THRESHOLD = "auth_failure_threshold"
...
def _fingerprint(value: object) -> str:
    ...
    if re.fullmatch(r"[0-9a-f]{16}", value) is None:
        raise cv.Invalid("candidate_secret_fingerprint must be 16 lowercase hex characters")
    return value
...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        # The committed examples only reference a non-production !secret value.
        cv.Required(CONF_CANDIDATE_PASSWORD): cv.string_strict,
        cv.Required(CONF_CANDIDATE_CLIENT_ID): cv.string_strict,
        cv.Required(CONF_ANONYMOUS_CLIENT_ID): cv.string_strict,
        cv.Optional(CONF_CANDIDATE_GENERATION, default=1): cv.int_range(
            min=1, max=65535
        ),
        cv.Required(CONF_SECRET_FINGERPRINT): _fingerprint,
        cv.Optional(CONF_AUTH_FAILURE_THRESHOLD, default=3): cv.int_range(
            min=1, max=10
        ),
        cv.Optional(CONF_OBSERVATION_SUCCESS_THRESHOLD, default=3): cv.int_range(
            min=1, max=20
        ),
        cv.Optional(CONF_RETRY_COOLDOWN, default="300s"):
...
async def to_code(config: dict) -> None:
    ...
    cg.add(var.set_auth_failure_threshold(config[CONF_AUTH_FAILURE_THRESHOLD]))
    cg.add(
