#include "stage2d9_g3_prepare_executor.h"
...
static const char *const TAG = "gh_stage2d9_exec";
...
constexpr const char *PREPARE_SCHEMA = "GH2D9_PREPARE_V1";
constexpr const char *VERIFY_SCHEMA = "GH2D9_VERIFY_V1";
...
void Stage2D9CommandEnvelope::clear() {
...
float Stage2D9G3PrepareExecutor::get_setup_priority() const {
  return setup_priority::DATA;
}

bool Stage2D9G3PrepareExecutor::all_zero_hex_(const std::string &value) {
  return !value.empty() &&
         std::all_of(value.begin(), value.end(),
                     [](char character) { return character == '0'; });
}

bool Stage2D9G3PrepareExecutor::valid_lower_hex_(const std::string &value,
                                                size_t length) {
...
bool Stage2D9G3PrepareExecutor::valid_suffix_(const std::string &value) {
...
bool Stage2D9G3PrepareExecutor::decode_hex_32_(
...
bool Stage2D9G3PrepareExecutor::sha256_(
...
std::string Stage2D9G3PrepareExecutor::hex_(
...
bool Stage2D9G3PrepareExecutor::constant_equal_(const std::string &left,
...
bool Stage2D9G3PrepareExecutor::verify_partition_() {
...
bool Stage2D9G3PrepareExecutor::initialize_partition_() {
  ...
  if (!this->partition_initialized_)
    ESP_LOGE(TAG, "stage2d9_executor_partition_init=%s",
             esp_err_to_name(status));
  return this->partition_initialized_;
}

bool Stage2D9G3PrepareExecutor::namespace_exists_(bool *exists) {
...
bool Stage2D9G3PrepareExecutor::configure_runtime_() {
...
bool Stage2D9G3PrepareExecutor::inspect_empty_() {
...
bool Stage2D9G3PrepareExecutor::parse_command_(
    const std::string &line, Stage2D9CommandEnvelope *envelope) const {
  ...
  Stage2D9CommandEnvelope parsed;
...
Stage2D9G3PrepareExecutor::build_configuration_(
    const Stage2D9CommandEnvelope &envelope) const {
  ...
  config.candidate.broker_host = "stage2d9.invalid";
  config.candidate.broker_port = 8883;
  config.candidate.broker_tls_server_name = "stage2d9.invalid";
...
bool Stage2D9G3PrepareExecutor::expected_candidate_digest_(
    const Stage2D9CommandEnvelope &envelope, std::string *digest) const {
  IsolatedAcceptanceTestConfiguration config =
...
bool Stage2D9G3PrepareExecutor::verify_recovered_candidate_(
    const Stage2D9CommandEnvelope &envelope, uint32_t expected_generation) {
  ...
  const bool digest_ok = this->build_candidate_digest_(candidate, &observed_digest) &&
                         constant_equal_(observed_digest,
...
bool Stage2D9G3PrepareExecutor::execute_prepare_(
    Stage2D9CommandEnvelope *envelope) {
  if (envelope == nullptr || envelope->verify_only ||
      ...
      !this->test_key_provider_.load(envelope->persistence_key)) {
    return false;
  }
  ...
  IsolatedAcceptanceTestConfiguration config =
      this->build_configuration_(*envelope);
  if (!config.valid() ||
      !this->package_.load_test_configuration(std::move(config)) ||
      !this->authorization_binder_.grant(
  ...
  const auto &snapshot = this->package_.snapshot();
  const bool verified =
      prepared && snapshot.phase == IsolatedAcceptancePhase::PREPARED &&
      snapshot.failure == IsolatedAcceptanceFailure::NONE &&
      snapshot.active_generation == 0 && snapshot.candidate_generation == 1 &&
      snapshot.driver.persistence_status == "no_active_prepared" &&
      snapshot.driver.read_only_observed &&
      snapshot.driver.persistent_write_count > 0 &&
      !snapshot.driver.active_session_live &&
      !snapshot.driver.candidate_session_live &&
      ...
      this->verify_recovered_candidate_(*envelope, 1);
  if (!verified)
    return false;

  this->prepare_succeeded_ = true;
  ESP_LOGI(TAG,
           "stage2d9_g3_prepare=pass active_generation=0 "
...
bool Stage2D9G3PrepareExecutor::execute_verify_(
  ...
  if (envelope == nullptr || !envelope->verify_only ||
      ...
      !this->test_key_provider_.load(envelope->persistence_key) ||
      !this->package_.inspect_read_only()) {
    return false;
  }
  ...
  ESP_LOGI(TAG,
           "stage2d9_g3_verify=pass active_generation=0 "
           "candidate_generation=1 candidate_state=PREPARED "
...
void Stage2D9G3PrepareExecutor::emit_snapshot_(const char *label) const {
  const auto &snapshot = this->package_.snapshot();
  ESP_LOGI(
      TAG,
      "stage2d9_g3_%s_snapshot phase=%s command=%s failure=%s "
...
void Stage2D9G3PrepareExecutor::wipe_runtime_() {
  this->authorization_binder_.clear();
...
void Stage2D9G3PrepareExecutor::close_partition_() {
  if (!this->partition_initialized_)
    return;
  const esp_err_t status =
      nvs_flash_deinit_partition(this->partition_label_.c_str());
  if (status != ESP_OK)
    ESP_LOGW(TAG, "stage2d9_executor_partition_deinit=%s",
             esp_err_to_name(status));
  this->partition_initialized_ = false;
}

void Stage2D9G3PrepareExecutor::fail_closed_(const char *reason) {
  this->wipe_runtime_();
  this->close_partition_();
  this->awaiting_ = AwaitingCommand::NONE;
  this->terminal_ = true;
  ESP_LOGE(TAG, "stage2d9_g3_executor=fail reason=%s",
           reason == nullptr ? "unknown" : reason);
  this->mark_failed();
}

void Stage2D9G3PrepareExecutor::process_line_(std::string line) {
...
void Stage2D9G3PrepareExecutor::read_console_() {
  char buffer[64];
...
void Stage2D9G3PrepareExecutor::setup() {
  this->command_surface_enabled_ =
      valid_lower_hex_(this->unlock_digest_, 64) &&
      !all_zero_hex_(this->unlock_digest_);
  ESP_LOGI(TAG, "stage2d9_g3_executor_begin build_binding=%s",
           this->build_binding_.c_str());
  ESP_LOGI(
      TAG,
      "stage2d9_g3_executor_boundary execution_authorized=false "
  ...
  if (!this->verify_partition_() || !this->initialize_partition_() ||
      !this->configure_runtime_()) {
  ...
  if (!this->command_surface_enabled_) {
    ...
    this->close_partition_();
    this->terminal_ = true;
    ESP_LOGI(TAG, "stage2d9_g3_executor=locked command_surface=false");
    return;
  }
  ...
  if (namespace_exists) {
    this->awaiting_ = AwaitingCommand::VERIFY;
    ESP_LOGI(TAG,
             "stage2d9_g3_command_ready=VERIFY expected_schema=%s "
  ...
  this->awaiting_ = AwaitingCommand::PREPARE;
  ESP_LOGI(TAG,
           "stage2d9_g3_command_ready=PREPARE expected_schema=%s "
           "execution_authorized=false",
           PREPARE_SCHEMA);
}

void Stage2D9G3PrepareExecutor::loop() {
...
void Stage2D9G3PrepareExecutor::dump_config() {
  ESP_LOGCONFIG(TAG, "Stage2D9 G3 token-gated PREPARE executor:");
  ...
  ESP_LOGCONFIG(TAG, "  Command surface enabled: %s",
                this->command_surface_enabled_ ? "true" : "false");
  ESP_LOGCONFIG(TAG, "  Command attempted: %s",
