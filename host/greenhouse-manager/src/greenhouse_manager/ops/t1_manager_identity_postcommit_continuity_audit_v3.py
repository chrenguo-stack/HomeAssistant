...
_TARGET_WRAPPER_SCHEMA_PREFIX = "gh.m2.t1-manager-target-production-execution/"
...
def _expected_source_binding(
    ...
    expected_manager_version: str | None,
) -> tuple[str, str] | None:
    if expected_repository_sha is None and expected_manager_version is None:
        return None
    if (
        not isinstance(expected_repository_sha, str)
        or len(expected_repository_sha) != 40
        or any(character not in "0123456789abcdef" for character in expected_repository_sha)
        or not isinstance(expected_manager_version, str)
        or not expected_manager_version
    ):
        raise ValueError("execution source binding inputs are invalid")
    return expected_repository_sha, expected_manager_version
...
def _execution_result(
    ...
    *,
    ...
    if transaction_id is not None:
        if transaction_id != journal.get("transaction_id"):
            raise ManagerPostcommitContinuityAuditError(
                "manager production execution result binding is invalid"
            )
        binding_mode = "journal-transaction-id"
    else:
        source_binding = _expected_source_binding(
            expected_repository_sha,
            expected_manager_version,
        )
        legacy_required = {
            "status": "manager_identity_production_execution_succeeded",
            "repository_sha": source_binding[0] if source_binding else None,
            "manager_version": source_binding[1] if source_binding else None,
        ...
        if (
            not schema.startswith(_TARGET_WRAPPER_SCHEMA_PREFIX)
            or source_binding is None
            or any(
                document.get(field) != expected
                for field, expected in legacy_required.items()
            )
        ):
            raise ManagerPostcommitContinuityAuditError(
                "legacy manager execution wrapper binding is invalid"
            )
        binding_mode = "legacy-single-use-authorization"

    return document, binding_mode
...
def build_manager_identity_postcommit_continuity_audit(
    ...
    *,
    ...
    node_id: str,
    discovery_topic: str,
    expected_execution_repository_sha: str | None = None,
    expected_execution_manager_version: str | None = None,
    mqtt_port: int = 1883,
    timeout_s: float = 8.0,
    ...
    execution, binding_mode = _execution_result(
        execution_path,
        journal,
        expected_repository_sha=expected_execution_repository_sha,
        expected_manager_version=expected_execution_manager_version,
    )
    ...
    return {
        ...
        "checks": checks,
        "execution_result_binding_mode": binding_mode,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--execution-repository-sha")
    parser.add_argument("--execution-manager-version")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        report = build_manager_identity_postcommit_continuity_audit(
            ...
            expected_execution_repository_sha=args.execution_repository_sha,
            expected_execution_manager_version=args.execution_manager_version,
