...
def _load_schema(name: str) -> dict[str, Any]:
    raw = files("greenhouse_manager").joinpath("schemas", name).read_text(
        encoding="utf-8"
    )
...
def _validate(
    ...
    try:
        ...
        raise ProjectionProtocolError(
            f"invalid {label}{suffix}: {exc.message}"
        ) from exc
...
def parse_projection_request(
    ...
    *,
    ...
    if document["system_id"] != expected_system_id:
        raise ProjectionProtocolError(
            "request system_id does not match the configured system"
        )
    ...
    if document["projection_hash"] != calculated_hash:
        raise ProjectionProtocolError(
            "projection_hash does not match canonical projection payload"
        )
    ...
    *,
    ...
    if verified_at is not None:
        if (
            ...
            or verified_at.utcoffset() != UTC.utcoffset(verified_at)
        ):
