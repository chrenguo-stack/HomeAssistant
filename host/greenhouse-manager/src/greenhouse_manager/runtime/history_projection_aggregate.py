...
def _projection_validator() -> Draft202012Validator:
    schema_path = files("greenhouse_manager").joinpath(
        "schemas/gh.c06-hourly-projection-1.schema.json"
    )
...
def _source_set_sha256(records: list[dict[str, Any]]) -> str:
    ...
    for boot_id, seq, record_sha256 in prepared:
        ...
        if identity in seen:
            raise ProjectionContractError(
                "duplicate source record identity in projection input"
            )
        seen.add(identity)
        identities.append(
            {
...
def _validate_payload(payload: dict[str, Any]) -> None:
    errors = sorted(
        _projection_validator().iter_errors(payload),
    ...
    if errors:
        ...
        raise ProjectionContractError(
            f"projection payload schema violation at {location}: {first.message}"
        )
...
def aggregate_projection(
    ...
    if len(records) > MAX_SOURCE_RECORDS_PER_HOUR:
        raise ProjectionContractError(
            f"projection hour exceeds {MAX_SOURCE_RECORDS_PER_HOUR} source records"
        )
    try:
    ...
    for rule in MEASUREMENT_RULES:
        ...
        try:
            ...
            raise ProjectionContractError(
                f"{rule.key} aggregation overflowed"
            ) from exc
    ...
    idempotency_key = (
        f"{task.node_id}|{task.sample_hour}|v{task.projection_version}"
    )
    payload = {
