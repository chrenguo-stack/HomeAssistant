...
class SqliteRelayAuthorizationProvider:
    ...
    def _load_key_file(self, key_file: object, *, expected_sha256: object = None) -> bytes:
        ...
        try:
            ...
            if not stat.S_ISDIR(dir_info.st_mode) or stat.S_IMODE(dir_info.st_mode) & 0o077:
                raise RelayAuthorizationStoreUnavailable("key_directory_permissions_invalid")
            ...
            if not stat.S_ISREG(info.st_mode) or stat.S_IMODE(info.st_mode) & 0o077:
                raise RelayAuthorizationStoreUnavailable("key_file_permissions_invalid")
            if hasattr(os, "getuid") and info.st_uid != os.getuid():
                raise RelayAuthorizationStoreUnavailable("key_file_permissions_invalid")
            chunks: list[bytes] = []
            total = 0
        ...
        if expected_sha256 is not None and hashlib.sha256(key).hexdigest() != expected_sha256:
        ...
        with self._lock:
            ...
            try:
                ...
                if self._schema_version == _SCHEMA_VERSION_V1:
                    ...
                    accepted = (
                        epoch is not None and epoch["enabled"] == 1 and epoch["state"] in _RUNTIME_KEY_STATES
                    )
