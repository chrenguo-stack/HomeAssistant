...
响应为 `gh.pair.secure-offer/1`，不得包含 `PAIR_SECRET`。

首次成功 claim 将 session 绑定到节点源 IP；同一 IP 可幂等重试，其他 IP 获得冲突或不存在响应。终态 session 从网络 registry 中删除。
