...
`hardware_id`：`ghw-<platform>-<12 lowercase hex>`，制造后不变。  
`node_id`：manager 分配，同一 system 唯一。  
`pairing_id`：每次尝试唯一的 128 位 UUID。  
...
```json
{
  ...
  "factory_pop": "<128-bit-or-more base64url>",
...
```text
ghpair://v1?hid=ghw-c6-98a316a9f2f8&pop=<base64url>&model=greenhouse-wifi-c6
```

PoP 不得出现在 Topic、日志、诊断或 Home Assistant 状态。
...
salt   = SHA256(factory_pop)
info   = SHA256(canonical_transcript)
key    = HKDF-SHA256(shared, salt, info, 32)
```

challenge/response 使用 `HMAC-SHA256(factory_pop, canonical_transcript)`。bundle 使用 AES-256-GCM，AAD 至少包含 schema、pairing_id、hardware_id、generation。
...
有备份时恢复 system_id、manager_id、CA、Dynamic Security 和注册表，节点无需重新配对。无备份时必须逐台扫码，不得凭同一 Wi-Fi 静默接管。
