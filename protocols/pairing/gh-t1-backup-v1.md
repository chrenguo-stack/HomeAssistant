...
- 不停止、重启或修改现有容器；
- 不输出文件内容；
- manifest 只记录镜像身份、相对路径、大小和 SHA-256；
- 拒绝组/其他用户可访问的输出目录；
- 拒绝符号链接来源。
...
`drill` 将文件解压到 0700 临时目录，检查 SQLite 完整性，并使用备份中记录的精确 Mosquitto image ID 创建 `--network none` 临时容器。只有临时 Broker 能启动且数据库完整才算通过。无论成功失败均删除临时容器和目录，不覆盖真实卷。
