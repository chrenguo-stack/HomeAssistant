...
- 96-bit nonce 固定为 `SHA256(node_id || 0x00 || boot_id)[0:8] || uint32_be(seq)`。
- 同一 key epoch 下禁止复用 `boot_id + seq`；`seq` 达到 `2^32-1` 后必须停发并建立新 boot。
...
7. 使用已有 `node_id + boot_id + seq` 去重集合统一处理直连与中继路径。
...
epoch、nonce 不匹配、AEAD 失败、内外身份不一致、帧超限、非单跳或重复 `node_id + boot_id + seq`。

诊断只记录固定错误码和非敏感身份；不得记录密钥、明文、nonce 前像或完整密文。

## 7. 后续独立门

- Manager 正式入口和持久化 replay window；
