...
FINALIZER=ROOT/"tools/h3_n2_stage2d9r_g3r_d2_17_g07_physical_decision_marker_finalizer_20260731_v1.py"
...
class TestG07PhysicalDecision(unittest.TestCase):
 ...
 def test_marker_finalizer(self):
  with tempfile.TemporaryDirectory() as td:
   home=Path(td); marker=home/".local/state/greenhouse-stage2d9r/d2-17-g07-physical-decisions/D1-H3N2-STAGE2D9R-G3R-D2-17-G07-PHYSICAL-EXECUTION-20260731-01.json"; marker.parent.mkdir(parents=True)
   value={"decision_id":"D1-H3N2-STAGE2D9R-G3R-D2-17-G07-PHYSICAL-EXECUTION-20260731-01","status":"CONSUMED_PASS","marker_sha256":"stale"}; marker.write_text(json.dumps(value))
   cp=subprocess.run([sys.executable,"-B",str(FINALIZER)],env={**os.environ,"HOME":str(home),"PYTHONDONTWRITEBYTECODE":"1"},capture_output=True,text=True,check=False); self.assertEqual(cp.returncode,0,cp.stdout+cp.stderr)
   final=json.loads(marker.read_text()); digest=final.pop("marker_sha256"); self.assertEqual(digest,canonical(final)); self.assertEqual(marker.stat().st_mode & 0o777,0o600)
 def test_launcher(self):
  t=LAUNCHER.read_text(); self.assertIn('if [ "$#" -ne 0 ]',t); self.assertIn('DRIVER_RC=$?',t); self.assertIn('physical_decision_marker_finalizer',t); self.assertNotIn('exec "$PYTHON_BIN"',t)
