...
class TestG07PhysicalDecision(unittest.TestCase):
 ...
 def test_no_local_paths_or_package_mutation(self):
  text=DRIVER.read_text(); self.assertNotIn("/Users/",text); self.assertNotIn("chmod(execution_root",text); self.assertIn("verified_two_hop_command",text); self.assertIn("[str(shell),str(inner)",text)
