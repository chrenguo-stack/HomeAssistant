...
TOOL = ROOT / "tools/h3_n2_stage2d9r_g3r_d2_17_g08_target_mac_static_check_acceptance_contract_20260731_v1.py"
ACCEPTANCE = ROOT / "docs/acceptance/h3-n2-stage2d9r-g3r-d2-17-g08-target-mac-static-check-pass-20260731-v1.json"
PENDING = ROOT / "docs/decisions/h3-n2-stage2d9r-g3r-d2-17-g08-physical-execution-pending-20260731-v1.json"

spec = importlib.util.spec_from_file_location("g08_acceptance", TOOL)
...
def expect_failure(fn, value, code_prefix: str) -> None:
    try:
        fn(value)
    except module.G08AcceptanceError as exc:
...
def main() -> int:
    ...
    changed["physical_pending_binding_sha256"] = module.canonical_sha256(
        {key: value for key, value in changed.items() if key != "physical_pending_binding_sha256"}
    )
    expect_failure(module.verify_pending, changed, "PHYSICAL_PENDING_BINDING_DRIFT")

    public_text = "\n".join(
        path.read_text(encoding="utf-8")
        for path in (ACCEPTANCE, PENDING, TOOL)
    )
    for forbidden in ("/Users/", "ActiveTestRuns", "AUTHORIZATION.json", "BEGIN PRIVATE KEY"):
