...
SCRIPT = ROOT / "tools/h3_n2_stage2d9r_private_content_binding_probe_20260724_v2.py"
...
class PrivateContentBindingProbeContractTest(unittest.TestCase):
    ...
    @unittest.skipUnless(shutil.which("openssl"), "openssl unavailable")
    def test_broker_certificate_binding_uses_der_digest(self) -> None:
        openssl = Path(shutil.which("openssl") or "").resolve(strict=True)
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            key = root / "key.pem"
            ...
            der = subprocess.run(
                [str(openssl), "x509", "-in", str(cert), "-outform", "DER"],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                check=True,
            ).stdout
            self.assertEqual(
                probe.certificate_der_sha256(openssl, cert),
                hashlib.sha256(der).hexdigest(),
            )
            self.assertNotEqual(
                probe.certificate_der_sha256(openssl, cert),
                probe.sha256_file(cert),
            )

    def test_source_does_not_compare_broker_pem_bytes_to_der_binding(self) -> None:
        text = SCRIPT.read_text(encoding="utf-8")
        self.assertNotIn(
            'sha256_file(broker_cert) == EXPECTED["broker_certificate_sha256"]',
            text,
        )
        self.assertIn('certificate_der_sha256(openssl, broker_cert)', text)
