#!/usr/bin/env python3
"""Fail-closed PR431 Board B preflight and minimal app+otadata writer.

Public source contains only public-safe hashes. Raw serial port, ROM MAC and
esptool output are captured in-process and are never written to tracked files.
"""
...
SCHEMA_PREFLIGHT = "n3w.kf096.pr431.boardb-preflight/1"
SCHEMA_WRITE = "n3w.kf096.pr431.boardb-write/1"

PRODUCT_SOURCE = "d1b5c3acbd32cca95483743ffe2edba9aa3f904f"
PRODUCT_TREE = "3f161c1550e1df48db7cd5a5970db1b11932bef0"
...
ARTIFACT_ZIP_SHA256 = "94ff922ce50314ed6f3275376eb5b1e71ae27f27f6edd16f9dd0743259dd13b6"
...
# Public-safe Board B identity authority from the R2B closeout.
EXPECTED_HARDWARE_ID_SHA256 = (
...
WRITE_CONFIRMATION = "PR431_BOARD_B_WRITE_AUTHORIZED"
...
def run_capture(args: list[str], *, port: str | None = None) -> str:
    ...
    if proc.returncode != 0:
        raise StopExecution(
            f"command failed rc={proc.returncode}: {_redacted_command(args, port)}"
        )
    return output
...
def run_preflight(args: argparse.Namespace) -> int:
    with tempfile.TemporaryDirectory(prefix="n3w-pr431-boardb-") as td:
...
def run_write(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="n3w-pr431-boardb-write-") as td:
        files = validate_artifact(Path(args.artifact_zip), Path(td))
        esptool_version = verify_esptool_version()
        verify_image(files["application"])

        # Freshly rebind the target immediately before mutation.
        board = probe_board(args.port)
