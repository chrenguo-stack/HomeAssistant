...
SCHEMA_PREFLIGHT = "n3w.production.gwsel-v1.board-c-preflight/1"
BOARD_LABEL = "C"
...
# Physical target binding uses a fresh ROM identity and requires it to match the
# frozen public-safe Board C hardware identity. The current application version is
# intentionally not constrained: A/B/C may begin this gate on different historical
# firmware revisions.
...
TARGET_CONFIRMATION = "BOARD_C_CONNECTED_FOR_GWSEL_V1_READONLY_PREFLIGHT"
PREFLIGHT_MAX_AGE_SECONDS = 900
EXPECTED_HARDWARE_ID_SHA256 = "d6ef3f98a35f06a5a8b8e7716a015e17336242128b314a1b3244b114fc6f72e2"
...
def probe_board(port: str) -> dict[str, object]:
    ...
    if identity_hash != EXPECTED_HARDWARE_ID_SHA256:
        raise StopExecution("connected target does not match frozen Board C identity")
...
def run_preflight(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="n3w-gwsel-v1-board-c-preflight-") as td:
    ...
    print("N3W_PRODUCTION_GWSEL_V1_BOARD_C_WRITE_TARGET_PREFLIGHT=PASS")
    ...
    print("HISTORICAL_IDENTITY_MATCH=true")
