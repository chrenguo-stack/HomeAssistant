#!/usr/bin/env python3
"""Host fault matrix for Stage2D8 dedicated-board G2 V64 artifact guards."""
...
def load_packager(path: Path) -> ModuleType:
    spec = importlib.util.spec_from_file_location("stage2d8_g2_packager_v64", path)
    ...
    *, readonly: bool = True, test_offset: int = 0x400000, extra: bool = False
    ...
    entries = [
        ...
        partition_entry(
            ...
            test_offset,
            0x10000,
            READONLY_FLAG if readonly else 0,
        ),
    ]
    if extra:
        entries.append(
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="stage2d8-g2-v64-") as temporary:
        root = Path(temporary)
        ...
        valid_seed.write_bytes(valid_prefix + b"\xff" * (0x10000 - len(valid_prefix)))
        results.append(
            expect_pass(
    ...
    report = {
        "schema": "gh.h3.n2.stage2d8-g2-host-fault-matrix/2",
        "status": "pass",
        "gate": "LOCKED",
