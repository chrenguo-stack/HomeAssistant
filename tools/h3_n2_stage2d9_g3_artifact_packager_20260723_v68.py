#!/usr/bin/env python3
"""Assemble immutable Stage2D9 G3 V68 executor and locked recovery artifacts."""
...
def verify_partition_table(path: Path) -> list[dict[str, int | str]]:
    ...
    require(set(by_label) == set(expected), "partition labels differ from V68 plan")
...
def main() -> int:
    ...
    require(boundary.get("artifact_generation") == "V68", "boundary generation mismatch")
    require(
        boundary.get("unlock_digest_sha256") == EXPECTED_UNLOCK_DIGEST,
        "boundary unlock digest mismatch",
    )
    require(repro.get("status") == "pass", "reproducibility did not pass")
    require(repro.get("artifact_generation") == "V68", "repro generation mismatch")
    ...
    build_names = {"g3": "gh-2d9-g3-v68", "recovery": "gh-2d9-recovery-v65"}
    ...
    for role, build_name in build_names.items():
        ...
        merged = role_dir / f"stage2d9-{role}-merged-v68.bin"
        run_checked(
            [
                str(esptool_python),
                "-m",
    ...
    execution_config = board / "greenhouse_profile_isolated_device_g3_execution_20260723_v68.yml"
    ...
    require(unlock_digest == EXPECTED_UNLOCK_DIGEST, "unexpected V68 unlock digest")
    manifest = {
        "schema": "gh.h3.n2.stage2d9-g3-artifact-manifest/2",
        ...
        "retired_v67_d2_material_reusable": False,
        "target": {
            "chip": "ESP32-C6",
    ...
    manifest_path = output / "stage2d9-g3-artifact-manifest-v68.json"
