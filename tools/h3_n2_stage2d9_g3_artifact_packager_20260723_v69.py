#!/usr/bin/env python3
"""Assemble corrected Stage2D9 G3 V69 and locked recovery artifacts."""
...
EXPECTED_IMPLEMENTATION_BINDING = "f39c3c4c621717a61e0b3cef8b4ec88e59ac13aa"
...
def verify_partition_table(path: Path) -> list[dict[str, int | str]]:
    ...
    require(set(by_label) == set(expected), "partition labels differ from V69 plan")
...
def scan_redaction(paths: list[Path]) -> None:
    ...
    for path in paths:
        ...
        if path.suffix.lower() in {".json", ".csv", ".txt", ".yml"} or path.name == "SHA256SUMS":
            for token in forbidden_text:
                require(token not in data,
                        f"private command material in {path.name}")
...
def main() -> int:
    ...
    require(boundary.get("artifact_generation") == "V69",
            "boundary generation mismatch")
    require(boundary.get("root_cause_fix") == "valid_local_placeholder_host",
            "root-cause correction missing")
    require(boundary.get("implementation_source_binding") == EXPECTED_IMPLEMENTATION_BINDING,
            "implementation binding mismatch")
    require(boundary.get("unlock_digest_sha256") == EXPECTED_UNLOCK_DIGEST,
            "boundary unlock digest mismatch")
    require(repro.get("status") == "pass", "reproducibility did not pass")
    require(repro.get("artifact_generation") == "V69",
            "repro generation mismatch")
    require(repro.get("byte_identical") is True,
            "builds are not byte-identical")
    require(repro.get("clean_build_count") == 2,
            "wrong clean build count")
    require(runtime.get("generator_version") == GENERATOR_VERSION,
            "generator version mismatch")
    require(runtime.get("wheel_sha256") == GENERATOR_WHEEL_SHA256,
            "generator wheel mismatch")

    output.mkdir(parents=True)
    board = repo / "firmware/esphome_rc/board_lab/h3_profile_isolated_device_g3_prepare"
    build_names = {"g3": "gh-2d9-g3-v69", "recovery": "gh-2d9-recovery-v65"}
    ...
    for role, build_name in build_names.items():
        ...
        if partition_entries is None:
            ...
            require(entries == partition_entries,
                    "G3/recovery partition binaries differ")
        merged = role_dir / f"stage2d9-{role}-merged-v69.bin"
        run_checked(
            [
                str(esptool_python),
                "-m",
    ...
    metadata_sources = (
        board / "stage2d9_g3_partitions_20260722_v65.csv",
        board / "stage2d9_g3_nvs_seed_20260722_v65.csv",
        board / "greenhouse_profile_isolated_device_g3_execution_20260723_v69.yml",
        board / "greenhouse_stage2d9_locked_recovery_20260722_v65.yml",
        boundary_path,
        repro_path,
        runtime_path,
        requirements,
    ...
    require(HEX64.fullmatch(unlock_digest) is not None,
            "unlock digest missing")
    require(unlock_digest == EXPECTED_UNLOCK_DIGEST,
            "unexpected V69 unlock digest")

    manifest = {
        "schema": "gh.h3.n2.stage2d9-g3-artifact-manifest/3",
        "artifact_generation": "V69",
        "gate": "LOCKED",
        "source_commit": args.source_commit,
        "implementation_source_binding": EXPECTED_IMPLEMENTATION_BINDING,
        ...
        "retired_v67_v68_d2_material_reusable": False,
        "target": {
            "chip": "ESP32-C6",
    ...
    manifest_path = output / "stage2d9-g3-artifact-manifest-v69.json"
