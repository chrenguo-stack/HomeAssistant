#!/usr/bin/env python3
"""Content-bound G14 physical runtime entry."""
...
G13_REPAIR="h3_n2_stage2d9r_g3r_d2_17_g13_existing_empty_baseline_and_claim_state_repair_20260731_v1"
def load(path:Path)->dict:
 v=json.loads(path.read_text(encoding="utf-8"))
 if not isinstance(v,dict):raise RuntimeError("JSON_OBJECT_REQUIRED")
 return v
def main()->int:
 p=argparse.ArgumentParser();p.add_argument("--execution-root",type=Path,required=True);p.add_argument("--physical-request",type=Path,required=True);p.add_argument("--identity-path",type=Path,required=True);p.add_argument("--identity-adapter-path",type=Path,required=True);p.add_argument("--marker-adapter-path",type=Path,required=True);p.add_argument("--g13-repair-path",type=Path,required=True);p.add_argument("--prepare-evidence-root",type=Path,required=True);p.add_argument("--delivery-evidence-root",type=Path,required=True);p.add_argument("--terminalization-evidence-root",type=Path,required=True);p.add_argument("remainder",nargs=argparse.REMAINDER);a=p.parse_args()
 ...
 execution=a.execution_root.expanduser().resolve(strict=True);paths=[a.identity_adapter_path,a.marker_adapter_path,a.g13_repair_path];resolved=[]
 for raw in paths:
 ...
 wrapper=importlib.import_module(WRAPPER);contract=importlib.import_module(CONTRACT);identity_adapter=importlib.import_module(IDENTITY_ADAPTER);marker_adapter=importlib.import_module(MARKER_ADAPTER);g13=importlib.import_module(G13_REPAIR)
 ...
 def configured():
  core=original();report=g13.install_baseline_work_directory_compatibility_repair(core)
  if not report.get("installed"):raise RuntimeError("G13_BASELINE_COMPATIBILITY_INSTALL_FAILED")
  return core
