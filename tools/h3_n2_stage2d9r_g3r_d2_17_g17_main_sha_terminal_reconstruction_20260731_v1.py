#!/usr/bin/env python3
"""Source-only G17 result alias repair and host-only terminal reconstruction.
...
G17_PENDING_BINDING_SHA256 = "903e5547c551c8fffdfe62dfce3b33ea2406bd16df16cbcb3d04867e88074317"
...
def authorization_with_main_sha_alias(authorization: dict[str, Any]) -> dict[str, Any]:
    ...
    if main_sha is not None:
        require(isinstance(main_sha, str) and HEX40.fullmatch(main_sha) is not None,
...
def install_main_sha_result_alias_repair(core: Any) -> dict[str, Any]:
    """Wrap the active result generator without changing authorization persistence."""
    original = getattr(core, "result_object", None)
    require(callable(original), "RESULT_OBJECT_NOT_CALLABLE")

    def repaired_result_object(**kwargs: Any) -> dict[str, Any]:
        call = dict(kwargs)
        call["authorization"] = authorization_with_main_sha_alias(
            call.get("authorization")
        )
        value = original(**call)
        require(isinstance(value, dict), "RESULT_OBJECT_NOT_MAPPING")
        return value
...
def reconstruct_terminal(g16_terminal: dict[str, Any]) -> dict[str, Any]:
    """Reconstruct the physical outcome from the authenticated G16 evidence."""
    ...
    value: dict[str, Any] = {
        "schema": "gh.h3.n2.stage2d9r-g3r-d2-17-g17-reconstructed-physical-terminal/1",
        "status": "PASS",
        ...
        "g17_pending_binding_sha256": G17_PENDING_BINDING_SHA256,
        "root_cause": "RESULT_GENERATOR_EXPECTED_MAIN_SHA_ALIAS_MISSING",
        "repair": "COPY_VALID_REPOSITORY_HEAD_SHA_TO_MAIN_SHA_BEFORE_RESULT_GENERATION",
...
def source_boundary() -> dict[str, Any]:
    return {
        "state": "SOURCE_ONLY_REQUIRES_EXACT_HOST_ONLY_CLOSURE_AUTHORIZATION",
        "board_operation": False,
        "usb_enumeration": False,
        "serial_operation": False,
        "esptool_operation": False,
