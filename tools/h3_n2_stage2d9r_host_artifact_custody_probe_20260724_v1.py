#!/usr/bin/env python3
"""Stage2D9R read-only public Artifact and private-custody metadata probe."""
...
"record":"fb11a086dcfb58ea483aae4d5c09f13c5740a5d619c4e967013a5da8e70fd44e"}
...
"record":"1764d202e294d4e3125d7f641f2a0068768f38104dfb6273bddfb3789268f770"}
...
def marker(p:Path,d:str,aid:str,status:str,record:str|None=None)->None:
    v=safe_json(p,d);q(v.get("authorization_id")==aid and v.get("status")==status and v.get("replay_permitted") is False,"marker binding mismatch")
    if record:q(v.get("record_sha256")==record,"marker record mismatch")
def command(home:Path)->dict[str,object]:
    ...
    marker(a/"U1-H3N2-STAGE2D9R-COMMAND-MATERIAL-20260724-02.consumed.json",CMD["u102"],"U1-H3N2-STAGE2D9R-COMMAND-MATERIAL-20260724-02","CONSUMED",CMD["record"])
    return {"root_digest":CMD["root"],"private_descriptor_sha256":CMD["private"],"public_descriptor_sha256":CMD["public"],"package_sha256":CMD["package"],"unlock_digest_sha256":U,"token_content_read":False}
def pki(home:Path)->dict[str,object]:
    ...
    a=(home/AUTH).resolve(strict=False);marker(a/"U1-H3N2-STAGE2D9R-PRIVATE-PKI-20260723-01.consumed.json",PKI["marker"],"U1-H3N2-STAGE2D9R-PRIVATE-PKI-20260723-01","CONSUMED",PKI["record"])
    return {"root_digest":PKI["root"],"private_descriptor_sha256":PKI["private"],"public_descriptor_sha256":PKI["public"],"package_sha256":PKI["package"],"ca_pem_sha256":CA,"candidate_digest_sha256":CAND,"private_key_content_read":False,"password_content_read":False}
def main()->int:
    ...
    try:
        ...
        im=immutable(root/"public-artifacts/immutable");reco=recovery(root/"public-artifacts/recovery")
        ...
        out={"schema":"gh.h3.n2.stage2d9r-host-artifact-custody-preauth-probe/1","stage":S,"result":"PASS_PUBLIC_ARTIFACTS_ONLY" if x.public_artifacts_only else "PASS_READ_ONLY_PREAUTH","python_executable_sha256":PY,"immutable_artifact":im,"recovery_artifact":reco,"command_custody":c,"pki_custody":pk,"private_material_content_binding":"NOT_EXECUTED_REQUIRES_SEPARATE_EXACT_AUTHORIZATION","private_content_read":False,"private_paths_included":False,"secret_values_included":False,"repository_required":False,"network_operation":False,"broker_started":False,"board_operation":False,"serial_operation":False,"flash_operation":False,"physical_nvs_operation":False,"prepare_executed":False,"verify_executed":False}
        print("HOST_ARTIFACT_CUSTODY_PREAUTH_PROBE=PASS");print(json.dumps(out,sort_keys=True));return 0
    except Exception as e:
        print("HOST_ARTIFACT_CUSTODY_PREAUTH_PROBE=FAIL");print(f"FAILURE_CLASS={type(e).__name__}");print(f"FAILURE_MESSAGE={e}")
