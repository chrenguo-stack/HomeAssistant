#!/usr/bin/env python3
"""Read-only metadata probe for the consumed Stage 2D-9R U1-03 marker.
...
AUTHORIZATION_ID = "U1-H3N2-STAGE2D9R-PRIVATE-CONTENT-BINDING-20260724-03"
EXPECTED_RECORD_SHA256 = "38edfa6ba1d42aea5d0d6d57f0a3157209be3afc6873f957d92d0156fceae0a2"
EXPECTED_FAILURE_CODE = "BROKER_CERTIFICATE_DIGEST_MISMATCH"
...
def probe(home: Path) -> dict[str, Any]:
    ...
    require(path.is_file() and not path.is_symlink(), "U1_03_MARKER_MISSING")
    require(file_mode(path) == "0600", "U1_03_MARKER_MODE_MISMATCH")
    raw = path.read_bytes()
    value = json.loads(raw)
    require(isinstance(value, dict), "U1_03_MARKER_TYPE_INVALID")
    require(
        value.get("schema")
        == "gh.h3.n2.stage2d9r-private-content-binding-u1-consumption/1",
        "U1_03_MARKER_SCHEMA_MISMATCH",
    )
    require(value.get("authorization_id") == AUTHORIZATION_ID, "U1_03_AUTHORIZATION_ID_MISMATCH")
    require(value.get("status") == "CONSUMED_FAILED", "U1_03_STATUS_MISMATCH")
    require(value.get("record_sha256") == EXPECTED_RECORD_SHA256, "U1_03_RECORD_MISMATCH")
    require(value.get("failure_code") == EXPECTED_FAILURE_CODE, "U1_03_FAILURE_CODE_MISMATCH")
    require(value.get("result_sha256") is None, "U1_03_RESULT_SHAPE_INVALID")
    require(value.get("one_shot") is True, "U1_03_ONE_SHOT_MISMATCH")
    require(value.get("replay_permitted") is False, "U1_03_REPLAY_MISMATCH")
    require(value.get("automatic_retry_permitted") is False, "U1_03_RETRY_MISMATCH")
    require(value.get("secret_values_included") is False, "U1_03_SECRET_FLAG_MISMATCH")
    claimed = parse_utc(value.get("claimed_at"), "claimed_at")
    consumed = parse_utc(value.get("consumed_at"), "consumed_at")
    require(consumed >= claimed, "U1_03_TIMESTAMP_ORDER_INVALID")
    digest = sha256_bytes(raw)
    require(HEX64.fullmatch(digest) is not None, "U1_03_MARKER_DIGEST_INVALID")

    return {
        "schema": "gh.h3.n2.stage2d9r-private-content-failure-marker-probe/1",
...
def main() -> int:
    ...
    print("PRIVATE_CONTENT_BINDING_FAILURE_MARKER_PROBE_V1_BEGIN")
    try:
        ...
        print("PRIVATE_CONTENT_FAILURE_MARKER_PROBE=FAIL")
        ...
        print("PRIVATE_CONTENT_BINDING_FAILURE_MARKER_PROBE_V1_END")
        return 2
    print("PRIVATE_CONTENT_FAILURE_MARKER_PROBE=PASS")
