#!/usr/bin/env python3
"""One-shot offline generator for Stage 2D-9R successor private execution material.

The default operation is a read-only toolchain probe. Secret generation requires
an exact, unexpired U1 authorization record and ``--execute``. The generator
never starts a Broker, opens a socket or serial port, accesses a board, writes
Flash/NVS, or sends PREPARE/VERIFY.
"""
...
AUTH_SCHEMA = "gh.h3.n2.stage2d9r-private-execution-material-successor-u1-authorization/1"
AUTH_OPERATION = "GENERATE_SUCCESSOR_PRIVATE_EXECUTION_MATERIAL"
AUTH_PREFIX = "U1-H3N2-STAGE2D9R-PRIVATE-EXECUTION-MATERIAL-"
STAGE = contract.STAGE
CUSTODY_RULE = "HOME_LOCAL_STATE_STAGE2D9R_PRIVATE_EXECUTION_MATERIAL_TLSVALID02"
...
ROOT_CA_CN = "Stage2D9R Successor Test Root CA"
ROOT_VALID_DAYS = 365
LEAF_VALID_DAYS = 30
...
class GenerationError(RuntimeError):
    """Fail-closed successor generation error."""
...
def resolve_executable(name: str, explicit: Path | None = None) -> Path:
    ...
    require(path.is_file() and os.access(path, os.X_OK), f"EXECUTABLE_INVALID_{name.upper()}")
    return path
...
def validate_private_root(root: Path, home: Path, repository_root: Path | None) -> None:
    ...
    require(root.resolve(strict=False) == default_custody_root(home_resolved), "CUSTODY_RULE_MISMATCH")
    ...
    if repository_root is not None:
        ...
        try:
            ...
            raise GenerationError("CUSTODY_ROOT_INSIDE_REPOSITORY")
...
def validate_authorization(
    ...
    require(record.get("run_suffix") == contract.RUN_SUFFIX, "AUTH_RUN_SUFFIX_MISMATCH")
    require(record.get("custody_root_selection_rule") == CUSTODY_RULE, "AUTH_CUSTODY_RULE_MISMATCH")
...
def build_acl() -> str:
    return (
        ...
        f"topic readwrite gh-test/gh-test-run-{contract.RUN_SUFFIX}/node/#\n"
    )
...
def validate_offline_pki(root: Path, openssl: Path) -> None:
    ...
    broker_cert_pub_pem = run_checked(
        [str(openssl), "x509", "-in", str(broker_cert), "-pubkey", "-noout"]
    )
    broker_cert_pub = run_checked(
        [str(openssl), "pkey", "-pubin", "-outform", "DER"],
        input_bytes=broker_cert_pub_pem,
    )
...
def material_metadata(root: Path) -> dict[str, dict[str, str]]:
    ...
    for filename in contract.REQUIRED_PRIVATE_FILES:
        path = root / filename
        require(path.is_file() and not path.is_symlink(), "PRIVATE_MATERIAL_MISSING")
        require(file_mode(path) == "0600", "PRIVATE_MATERIAL_MODE_INVALID")
        result[filename] = {
...
def toolchain_public_summary(toolchain: Toolchain, home: Path) -> dict[str, object]:
    root = default_custody_root(home)
    return {
        "schema": "gh.h3.n2.stage2d9r-private-execution-material-successor-toolchain-probe/1",
        "stage": STAGE,
...
def execute_generation(
    ...
    try:
        ...
        protocol_verify = protocol.render_verify(
            contract.RUN_SUFFIX,
            unlock_token,
            persistence_key,
            candidate_sha,
        ) + "\n"
        require(prepare == protocol_prepare, "PREPARE_RENDER_CROSS_BINDING_FAILED")
        require(verify == protocol_verify, "VERIFY_RENDER_CROSS_BINDING_FAILED")
        ...
        require(
            ...
            and parsed_prepare.authorization_digest == mqtt_password,
        ...
        public_descriptor = contract.build_public_descriptor(
            source_sha,
            sha256_bytes(mqtt_password.encode("ascii")),
            unlock_digest,
            sha256_file(root / PRIVATE_FILENAMES["persistence_key"]),
            sha256_file(ca_path),
            cert_der_sha,
            broker_spki_sha,
            candidate_sha,
            sha256_file(root / PRIVATE_FILENAMES["prepare_command"]),
            sha256_file(root / PRIVATE_FILENAMES["verify_command"]),
            private_package_sha,
        )
        public_bytes = (
        ...
        finalize_authorization(
            marker, "CONSUMED", public_descriptor_sha256=public_sha
        )
        mqtt_password = "0" * len(mqtt_password)
        persistence_key = "0" * len(persistence_key)
        unlock_token = "0" * len(unlock_token)
        return {
            "schema": "gh.h3.n2.stage2d9r-private-execution-material-successor-generation-result/1",
            "status": "PASS",
            "authorization_id": authorization_id,
            "source_sha": source_sha,
            "run_suffix": contract.RUN_SUFFIX,
        ...
        code = exc.args[0] if isinstance(exc, (GenerationError, contract.ContractError)) and exc.args else type(exc).__name__
        if marker.exists():
            finalize_authorization(marker, "CONSUMED_FAILED", failure_code=str(code))
        raise
...
def main() -> int:
    ...
    try:
        toolchain = probe_toolchain(
            generator_path, contract_path, protocol_path, args.openssl, args.mosquitto_passwd
        )
        if args.probe_toolchain and not args.execute:
            print(
                "STAGE2D9R_SUCCESSOR_PRIVATE_EXECUTION_MATERIAL_TOOLCHAIN_PROBE=PASS"
            )
        ...
        code = exc.args[0] if isinstance(exc, (GenerationError, contract.ContractError)) and exc.args else type(exc).__name__
        print("STAGE2D9R_SUCCESSOR_PRIVATE_EXECUTION_MATERIAL_GENERATION=FAIL")
        ...
        return 2
    print("STAGE2D9R_SUCCESSOR_PRIVATE_EXECUTION_MATERIAL_GENERATION=PASS")
